...
if __name__ == "__main__":
    ...
    autoencoders = CAVAutoencoder(input_dims=[len(cav[0]) for cav in cavs], embed_dim=embed_dim, device=device).to(device)
    autoencoders.train_autoencoders(cavs=cavs, save_dir=os.path.join(save_dir,model_to_run), overwrite=False, epochs=3) #train autoencoder/ we need decoders to reconstruct the cavs for each layer
    
    integrate_cav = IntegrateCAV(cavs=cavs, device=device, autoencoders=autoencoders,dim_align_method=dim_align_method).to(device)
    # align before fusion
    aligned_cavs = integrate_cav.align_with_moco(queue_size=256, momentum=0.999, temperature=0.07, embed_dim=embed_dim,overwrite=True, epochs=13,save_dir=os.path.join(save_dir,model_to_run))
